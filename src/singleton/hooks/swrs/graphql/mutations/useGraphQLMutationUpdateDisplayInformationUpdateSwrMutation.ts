...
export type UseGraphQLUpdateDisplayInformationMutationArgs =
  MutationUpdateDisplayInformationParams;

export const useGraphQLMutationUpdateDisplayInformationSwrMutation =
  (): UseSWRMutation<
    GraphQLResponse,
  ...
  > => {
      const swrMutation = useSWRMutation(
          ...
          async (
              ...
              if (!result.data) {
                  throw new Error(
                      "No data returned from update display information mutation"
                  )
              }
