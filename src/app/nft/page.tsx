...
import { toast, useGraphQLMutationFreezeSolanaMetaplexNFTSwrMutation, useGraphQLMutationValidateSolanaMetaplexNFTFrozenSwrMutation, useTransferNFTFormik } from "@/hooks"
import { useSingletonHook } from "@/modules/singleton-hook"
import { setSignTransactionModal, setTransferTab, TransactionType, TransferTab, useAppDispatch, useAppSelector } from "@/redux"
import { SendHorizonalIcon, PackageIcon, PackageOpenIcon, WandSparklesIcon, EyeIcon, CircleCheckBigIcon } from "lucide-react"
...
const Page: FC = () => {
    ...
    const nftAddress = useAppSelector(
        (state) => state.sessionReducer.nftAddress
    )
    ...
    const nft = collectionSwr.data?.nfts.find((nft) => nft.nftAddress === nftAddress)
    const dispatch = useAppDispatch()
    const network = useAppSelector(
        (state) => state.sessionReducer.network
    )
    const formik = useSingletonHook<ReturnType<typeof useTransferNFTFormik>>(TRANSFER_NFT_FORMIK)
    ...
    const rarity = nft.attributes.find((attribute) => attribute.key === AttributeName.Rarity)?.value as NFTRarityEnum
    return (
        <Container hasPadding>
            <div className="h-full">
                                ...
                                onClick={async () => {
                                    ...
                                    const { data } = await freezeSolanaMetaplexNFTSwrMutation.trigger({
                                        request: {
                                            ...
                                            collectionAddress: collections[collectionKey]?.collectionAddress
                                        }
                                    })
                                    if (!data) {
                                        toast({
                ...
                <Spacer y={12}/>
                <Title title="Stats" tooltipString="Stats are the attributes of the NFT. They are used to determine the rarity of the NFT." />
                <Spacer y={4}/>
                <List enableScroll={false} items={Object.values(StatsAttributeName)} contentCallback={(name) => {
                    const attribute = nft.attributes.find((attribute) => attribute.key === name)
                    return (
                        <div className="px-3 py-2">
                            <div className="flex gap-2 items-center justify-between w-full">
                                <Title title={statsAttributeNameMap[name].name} tooltipString={statsAttributeNameMap[name].tooltip}
                                    classNames = {{
                                        title: "text-sm",
