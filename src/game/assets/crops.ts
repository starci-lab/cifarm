...
export const cropAssetMap: Record<CropId, CropAssetData> = {
    [CropId.Carrot]: {
        ...
        dir: "carrot",
        stages: {
            0: { offsets: { x: 0, y: 0 }, extension: "png" },
            1: { offsets: { x: 0, y: 0 }, extension: "png" },
            2: { offsets: { x: 0, y: 0 }, extension: "png" },
            3: { offsets: { x: 0, y: 0 }, extension: "png" },
            4: { offsets: { x: 0, y: -40 }, extension: "png" },
        },
        name: "Carrot",
    },
    [CropId.BellPepper]: {
    ...
    Object.keys(cropAssetMap).forEach((cropIdKey) => {
        ...
        for (let i = 0; i < NUM_GROWTH_STAGES; i++) {
            scene.load.image(
                ...
                `${BASE_DIR}/${cropData.dir}/${i + 1}.${cropData.stages[i].extension}` // Assuming growth stages are numbered 1, 2, 3, ...
            )
        }
...
export const getCropAssetKey = ({
    ...
    return `crops-${cropId}-${growthStage}`
}

// Function to get the GID for a specific crop and its growth stage
export const getCropGid = ({
    cropId,
    growthStage,
}: GetCropGidParams): number => {
...
export type GetCropGidParams = GetCropAssetKeyParams;
