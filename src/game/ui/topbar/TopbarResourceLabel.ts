...
import { TextColor, Text } from "../elements/Text"

export interface TopbarResourceLabelOptions {
    iconKey: string
    text: string
    //icon scale
    scale?: number      
...
export class TopbarResourceLabel extends Label {
    constructor({
        baseParams: { scene, config },
        options,    
    }: ConstructorParams<LabelBaseConstructorParams, TopbarResourceLabelOptions>) {
        if (!options) {
